...
| Variable                    | Meaning                                                                                                                                                                                                                                                                                       |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`AWS_ACCESS_KEY_ID`**     | AWS access key for boto3 S3 calls (presigned URLs, uploads). Optional when using IAM/instance credentials or another auth mechanism supported by boto3. Empty values are treated as unset.                                                                                                    |
| **`AWS_SECRET_ACCESS_KEY`** | Secret key paired with `AWS_ACCESS_KEY_ID`.                                                                                                                                                                                                                                                   |
| **`AWS_REGION`**            | AWS region for the S3 client. Defaults to **`ap-south-1`** if unset or empty ([`src/utils.py`](src/utils.py)).                                                                                                                                                                                |
| **`S3_OUTPUT_BUCKET`**      | **Required** bucket name for artifacts (uploads, job outputs). Missing values cause [`get_s3_output_config()`](src/utils.py) to raise when presigning or uploading.                                                                                                                           |
